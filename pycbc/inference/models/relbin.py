...
from pycbc.waveform.ias_tf2 import hf3hPN
from pycbc.waveform.imr_phenomd import generate
...
class Relative(BaseDataModel):
    ...
    def __init__(self, data, psds, mass1, mass2, spin1z, spin2z,
        ...
        dt = {ifo: self.det[ifo].time_delay_from_earth_center(self.ra, self.dec, self.tc)
              for ifo in data}
        ...
        if lambda_tilde != 0.:
            logging.info("Using IAS TF2 template")
            eta = eta_from_mass1_mass2(mass1, mass2)
            m_total = mass1 + mass2
            self.h00 = hf3hPN(self.f, m_total, eta,
                              s1z=spin1z, s2z=spin2z, Lam=lambda_tilde)
        else:
            logging.info("Using IMR template")
            self.h00 = generate(self.f, mass1=mass1,
                                mass2=mass2, chi1=spin1z,
                                chi2=spin2z, fref=self.fref,
                                distance=1.)
    ...
    def waveform_ratio(self, p, htf, dtc=0.0):
        ...
        s1z = p['spin1z']
        s2z = p['spin2z']
        lam = 0. if 'lambda_tilde' not in p else p['lambda_tilde']
        if lam != 0.:
            # use IAS TF2 template
            m_total = p['mass1'] + p['mass2']
            eta = eta_from_mass1_mass2(p['mass1'], p['mass2'])
            htarget = hf3hPN(self.fedges, m_total, eta, s1z=s1z, s2z=s2z,
                             Lam=lam)
        else:
            # use IMR template
            htarget = generate(self.fedges, mass1=p['mass1'],
                               mass2=p['mass2'], chi1=p['spin1z'],
                               chi2=p['spin2z'], fref=self.fref)
